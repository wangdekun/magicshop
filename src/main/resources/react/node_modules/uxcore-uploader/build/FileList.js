...
var FileList = function (_React$Component) {
  ...
  FileList.prototype.onShowFile = function onShowFile(file, url, current) {
    if (this.props.isOnlyImg && url) {
      var fileList = this.props.fileList.map(function (item, index) {
        if (item.response) {
      ...
      var shows = fileList.filter(function (item) {
        return !!item;
      });
  ...
  FileList.prototype.render = function render() {
    return _react2["default"].createElement(
      'div',
      { className: 'kuma-upload-filelist ' + (this.props.mode === 'nw' ? 'nwmode' : this.props.mode === 'mini' ? 'minimode' : 'iconmode') + (this.props.isVisual ? ' filelist-visual' : '') },
