...
export default class FileItem extends React.Component {
  ...
  render() {
    ...
    if (this.props.mode === 'icon') {
      return (
        <div className={`kuma-upload-fileitem status-${this.state.status}`}>
          <a
            className="kuma-upload-action action-remove"
            onClick={this.onCancel.bind(this)}
            title={i18n[locale].remove}
          >
            <Icon name="shanchu" />
          </a>
          <div className="filepreview">
            ...
            {this.state.status === 'error' ? (
              <a
                className="kuma-upload-action action-retry"
                onClick={this.onPending.bind(this)}
                title={i18n[locale].retry}
              >
            ...
            {this.state.status === 'queued' ? (
              <a
                className="kuma-upload-action action-upload"
                onClick={this.onPending.bind(this)}
                title={i18n[locale].upload}
              >
                <i className="kuma-icon kuma-icon-triangle-right" />
              </a>
            ) : null}
            {this.state.status === 'progress' || this.state.status === 'pending' ? (
              <Progress file={this.file} interval={interval} percentage={this.state.percentage} />
            ) : null}
          </div>
          {this.state.status === 'error' ? (
            <a className="kuma-upload-status status-error" title={i18n[locale].upload_failed}>
              <i className="kuma-icon kuma-icon-caution" />
            </a>
          ) : null}
          {this.state.status === 'success' ? (
            <a className="kuma-upload-status status-success">
              <i className="kuma-icon kuma-icon-choose" />
            </a>
          ) : null}
          <div className="filename" title={this.file.name}>
      ...
      if (this.props.isOnlyImg) {
        if (!this.props.isVisual) {
          return (
            <div className={`kuma-upload-fileitem-img status-${this.state.status}`}>
              ...
              {this.state.status !== 'error' && this.state.status !== 'success' ? (
                <Progress file={this.file} interval={interval} />
              ) : null}
              <div className="field-image-name" title={this.file.name}>
                {this.file.name}
              </div>
        ...
        return (
          <div className={`kuma-upload-fileitem-visual status-${this.state.status}`}>
            <div className="field-image-info" />
          ...
          <label className="field-icon">
            {this.state.status === 'error' || this.state.status === 'success' ? (
              <i
                ...
                data-type={this.file.type}
              />
            ) : null}
          </label>
          {this.state.status !== 'error' && this.state.status !== 'success' ? (
            <Progress interval={interval} file={this.file} />
          ) : null}
          <div className="field-line" />
          <div className="field-info-wrap">
            <label className="field-info">
              <span className="filename" title={this.file.name}>
                {this.file.name}
              </span>
            </label>
            <label className="field-status">
              {this.state.status === 'error' ? (
                <a className="kuma-upload-status status-error">{i18n[locale].upload_failed}</a>
              ) : null}
              {this.state.status === 'success' && previewUrl ? (
                <a className="kuma-upload-action" target="_blank" href={previewUrl}>
                  {i18n[locale].preview}
                </a>
              ) : null}
              {this.state.status === 'success' && downloadUrl ? (
                <a className="kuma-upload-action" target="_blank" href={downloadUrl} download>
              ...
              {this.state.status === 'success' || this.state.status === 'error' ? (
                ...
                <a
                  className="kuma-upload-action terminal-action"
                  onClick={this.onCancel.bind(this)}
                >
                  <Icon name="guanbi" />
                </a>
